...
local function openMenu(data)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			...
			if ESX.Items[tostring(v["icon"])] then
				if not string.find(ESX.Items[tostring(v["icon"])].image, "images/") then
					img = img.."images/"
				end
				v["icon"] = img..ESX.Items[tostring(v["icon"])].image
			end
		end
	end
...
RegisterNetEvent('tiw-menu:client:openMenu', function(data)
    openMenu(data)
end)

RegisterNetEvent('tiw-menu:client:closeMenu', function()
    closeMenu()
end)
